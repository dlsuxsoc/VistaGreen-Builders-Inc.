...
function initializeSpecificPages(pathName, origin, callback){
...
function initializeGenericPages(pathName){
	...
	if((pathName.includes("index.php/vbi") || pathName.includes("index.php/vc")) &&
	   ...
	   !(pathName.includes("/projects") || pathName.includes("/careers") || 
		...
		for (i = 0; i < items.length; i++) {
			...
			var i2 = i + 1;
			while(idSection.toUpperCase() !== navItems[i2].innerText.replace(/ /g, '').concat("SECTION").toUpperCase())
				i2++;
			...
			if(i2 == i + 1)
			   ...
			   (function(i, i2, idSection) {
					new Waypoint({
						element: document.getElementById(idSection),
						handler: function(direction) {
							$('#navBarCollapsibleOptions li').removeClass('active');
							if(direction == 'down')
								 $('#navBarCollapsibleOptions li:nth-child(' + (i2 + 1) + ')').addClass('active');
							else {
								 // $('#navBarCollapsibleOptions li:nth-child(' + (i+1 + 1) + ')').removeClass('active');
								 if(i != 0)
									$('#navBarCollapsibleOptions li:nth-child(' + (i + 1) + ')').addClass('active');
					...
					$("#navBarCollapsibleOptions li:nth-child(" + (i2 + 1) + ")").click(function(e) {
						e.preventDefault();
						// console.log(i + " " + idSection);
						$('html,body').animate({
							scrollTop: $("#" + idSection ).offset().top},
...
$(document).ready(function(){
	...
	initializeSpecificPages(pathName, origin, function(){
		initializeGenericPages(pathName);
	});
